...
function getActivePlanAmounts() {
    return getDB().prepare('SELECT amount FROM plans WHERE is_active = 1').all().map(r => r.amount);
}
...
router.put('/members/:id', requireAuth, (req, res) => {
    ...
    if (name !== member.Name || phoneStr !== String(member.Phone)) {
        db.prepare('UPDATE payments SET Phone = ?, Name = ? WHERE Phone = ?').run(phoneStr, name, member.Phone);
        db.prepare('UPDATE daily_entry SET Phone = ?, Name = ? WHERE Phone = ?').run(phoneStr, name, member.Phone);
    }
    res.json({ success: true });
});
...
router.get('/stats/extended', requireAuth, (_req, res) => {
    ...
    res.json({ newThisMonth, topMembers, revenueByMode, avgDailyCheckins: Number(avgDaily), ytdRevenue });
});
